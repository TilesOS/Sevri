...
export async function POST(
  ...
  try {
    const limit = await enforceRateLimit({
      userId: user.id,
      endpoint: `portfolio_export:common_app_activity:${view.entry.id}`,
      maxRequests: 10,
      windowMinutes: 24 * 60,
    });
    if (!limit.allowed) {
    ...
    captureServerError(error, {
      route: "portfolio/exports/common-app",
      project_id: projectId,
    });
    return NextResponse.json(
      { error: error instanceof Error ? error.message : "Failed to generate Common App export." },
      { status: 400 },
    );
  }
}
