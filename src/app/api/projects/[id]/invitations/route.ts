...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  if (!perProjectLimit.allowed) {
    return NextResponse.json(
      { error: "Too many invitations for this project today. Try again tomorrow.", code: "rate_limited" },
      { status: 429 },
    );
  }
  ...
  if (!perInviterLimit.allowed) {
    return NextResponse.json(
      { error: "Too many invitations today. Try again tomorrow.", code: "rate_limited" },
      { status: 429 },
    );
  }
