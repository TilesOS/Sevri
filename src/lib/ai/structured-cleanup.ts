...
export function applyAndValidateStructuredCleanup<TSchema extends z.ZodTypeAny>(input: {
  ...
  const schemaResult = input.schema.safeParse(cleanup.cleaned);
  const schemaIssues = schemaResult.success
    ? []
    : schemaResult.error.issues.map(formatSchemaIssue);

  const candidate = schemaResult.success ? schemaResult.data : cleanup.cleaned;
  ...
  if (input.validator) {
    try {
      ...
      semanticIssues = [
        `Semantic validation failed after cleanup: ${
